...
const info = {
    name: 'wxpaynp',
    showname: '微信官方支付V3服务商版',
    author: '微信',
    link: 'https://pay.weixin.qq.com/',
    types: ['wxpay'],
    inputs: {
        appid: {
            name: '服务商公众号/小程序APPID',
            type: 'input',
            note: ''
        },
        appkey: {
        ...
        appurl: {
            name: '证书序列号',
            type: 'input',
            note: '在API安全-API证书中查看'
        },
        appswitch: {
            name: '子商户公众号/小程序APPID',
            type: 'input',
            note: '子商户在微信开放平台上申请的APPID，选填'
        }
    },
    select: {
        '1': 'Native扫码支付',
    ...
    note: '<p>请上传服务商商户API私钥 apiclient_key.pem</p>',
    bindwxmp: true,
    bindwxa: true
};
...
async function sendRequest(method, apiUrl, body, channelConfig) {
    ...
    const authorization = buildAuthHeader(method, url.pathname, bodyStr, channelConfig.appkey, channelConfig.appurl, privateKey);
...
async function qrcode(channelConfig, orderInfo, conf) {
    ...
    const body = {
        sp_appid: channelConfig.appid,
        sp_mchid: channelConfig.appkey,
        sub_mchid: channelConfig.appmchid,
        description: name,
    ...
    // 添加子商户APPID
    if (channelConfig.appswitch) {
        body.sub_appid = channelConfig.appswitch;
    }
    ...
    const result = await sendRequest('POST', '/v3/pay/partner/transactions/native', body, channelConfig);
...
async function h5pay(channelConfig, orderInfo, conf) {
    ...
    const result = await sendRequest('POST', '/v3/pay/partner/transactions/h5', body, channelConfig);
...
async function jspay(channelConfig, orderInfo, conf) {
    ...
    const result = await sendRequest('POST', '/v3/pay/partner/transactions/jsapi', body, channelConfig);
    ...
    // 使用子商户APPID或服务商APPID
    const payAppId = channelConfig.appswitch || channelConfig.appid;
...
async function apppay(channelConfig, orderInfo, conf) {
    ...
    const result = await sendRequest('POST', '/v3/pay/partner/transactions/app', body, channelConfig);
...
async function scanpay(channelConfig, orderInfo, conf) {
    ...
    const result = await sendRequest('POST', '/v3/pay/partner/transactions/codepay', body, channelConfig);
    ...
    if (result.trade_state === 'SUCCESS') {
        return {
            ...
            data: {
                ...
                buyer: result.payer?.sub_openid || result.payer?.sp_openid,
    ...
    try {
        ...
        if (data.trade_state === 'SUCCESS') {
            return {
                ...
                buyer: data.payer?.sub_openid || data.payer?.sp_openid
            };
        }
        
        return { success: false };
    } catch (error) {
        console.error('微信V3服务商版回调处理错误:', error);
        return { success: false };
    }
}
...
async function close(channelConfig, order) {
    ...
    await sendRequest('POST', `/v3/pay/partner/transactions/out-trade-no/${order.trade_no}/close`, body, channelConfig);
