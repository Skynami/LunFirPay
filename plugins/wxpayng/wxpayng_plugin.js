...
const info = {
    ...
    inputs: {
        ...
        appmchid: {
            name: '公众号/小程序Secret',
            type: 'textarea',
            note: '微信公众号或小程序的appsecret，使用jsapi支付必填'
        },
        appurl: {
            name: '证书序列号',
            type: 'input',
    ...
    note: '<p>请上传商户API私钥 apiclient_key.pem</p><p>此插件用于微信支付国际版商户（API网关为apihk.mch.weixin.qq.com）</p>',
    bindwxmp: true,
    bindwxa: true
};

const API_BASE = 'https://apihk.mch.weixin.qq.com';
...
async function apppay(channelConfig, orderInfo, conf) {
    ...
    const appParams = {
        appid: channelConfig.appid,
        partnerid: channelConfig.appkey,
...
async function notify(channelConfig, notifyData, order, headers) {
    try {
        ...
        console.error('微信国际版V3回调处理错误:', error);
        return { success: false };
    }
}
